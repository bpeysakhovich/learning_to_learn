...
class Stimulus:
    ...
    def generate_batch_task1(self, image_pair):
        ...
        batch_data   = np.zeros((par['batch_size'], trial_length, 32,32,3), dtype = np.float32)
        rewards = np.zeros((par['batch_size'], trial_length, par['layer_dims'][-1]), dtype = np.float32)
        ...
        for i in range(par['batch_size']):
            ...
            batch_data[i, range(fix, fix+stim), :, :, :] = np.reshape(self.test_images[image_ind,:,:,:], (1,32,32,3))
            # fixation
            rewards[i,range(0, fix+stim+delay), 1] = -1 # fixation break
            rewards[i,range(0, fix+stim+delay), 2] = -1 # fixation break
            # response
            batch_labels[i,range(fix + stim + delay, trial_length), sac_dir] = 1 # reward correct response
            batch_labels[i,range(fix + stim + delay, trial_length), 1+sac_dir%2] = -1 # penalize incorrect response
